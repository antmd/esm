...
namespace ESM
{
  ...
  OrderBook::OrderBook( ReplyApplication &replyApplication, OrderPtr order ) 
    ...
    _isActive( true )
  {
    _marketPictureRecord.setScripCode( order->getSecurityId() ) ;
  ...
  void OrderBook::replaceBuy( OrderPtr newOrder ) 
  {
    ...
    if( newOrder->getTimeInForce() == TimeInForce_IOC )
    {
      ...
      _replyApplication.sendCancelConfirm( canceledOrder, "IOC Order Cancelled Successfully" ) ;
    }
  }
  ...
  void OrderBook::insertBuy( OrderPtr buyOrder ) 
  {
    ...
    if( buyOrder->getPendingQty() > 0 )
    {
      ...
      {
        _replyApplication.sendCancelConfirm( buyOrder, "IOC Order Cancelled Successfully" ) ;
      }
      else
      {
        if( buyOrder->getOrderType() == OrderType_MARKET )
        {
          ...
          {
            _replyApplication.sendCancelConfirm( buyOrder, "Market Order Cancelled since we do not have a LTP" ) ;
            return ;
          }
        }
  ...
  void OrderBook::insertSell( OrderPtr sellOrder ) 
  {
    ...
    if( sellOrder->getPendingQty() > 0 )
    {
      ...
      {
        _replyApplication.sendCancelConfirm( sellOrder, "IOC Order Cancelled Successfully" ) ; 
      }
      else
      {
        if( sellOrder->getOrderType() == OrderType_MARKET )
        {
          ...
          {
            _replyApplication.sendCancelConfirm( sellOrder, "Market Order Cancelled since we do not have a LTP" ) ;
            return ;
          }
        }
  ...
  void OrderBook::checkTriggeredOrders( ) 
  {
    ...
    try
    {
      while( _marketPictureRecord.getLastTradePrice() >= _stopLossBuyOrders.first()->getStopPrice() )
      {
    ...
    try
    {
      while( _marketPictureRecord.getLastTradePrice() <= _stopLossSellOrders.first()->getStopPrice() )
      {
